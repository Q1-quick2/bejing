...
public class StringUtils {
    ...
    /**
     *
     * @Description 生成24位UUID,作为主键入库使用
     * @author pengwz
     * @date 2019年12月22日 上午11:37:16
     */
    public static String generateUUID() {
    	String uuid = UUID.randomUUID().toString();
    	String upperCase = uuid.substring(0,24).toUpperCase();
    	//生成随机下标,0代表生成数字,1代表生成字母 // 0-48   A-65
    	int nextInt = new Random().nextInt(2);
    	if(nextInt == 0) {
    		 int num = new Random().nextInt(10);
    		 upperCase = upperCase.replace("-", num+"");
    	}else {
